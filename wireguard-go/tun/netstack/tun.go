...
func CreateNetTUN(localAddresses, dnsServers []netip.Addr, mtu int) (tun.Device, *Net, error) {
	...
	dev := &netTun{
		...
		dnsCache:       newDNSCache(60*time.Second, 5*time.Minute),
		mtu:            mtu,
	}
...
func (net *Net) DialContextTCPAddrPort(ctx context.Context, addr netip.AddrPort) (*gonet.TCPConn, error) {
	...
	return dialContextTCPNoDelay(ctx, net.stack, fa, pn)
}
...
func (tnet *Net) LookupContextHost(ctx context.Context, host string) ([]string, error) {
	if tnet.dnsCache != nil && cacheableDNSName(host) {
		return tnet.dnsCache.lookup(ctx, host, tnet.lookupContextHostUncached)
	}
	return tnet.lookupContextHostUncached(ctx, host)
}

func cacheableDNSName(host string) bool {
	zlen := len(host)
	...
	if _, err := netip.ParseAddr(host[:zlen]); err == nil {
		return false
	}
	return isDomainName(host)
}

func (tnet *Net) lookupContextHostUncached(ctx context.Context, host string) ([]string, error) {
...
func newDNSCache(softTTL, hardTTL time.Duration) *dnsCache {
	return &dnsCache{
		softTTL: softTTL,
		hardTTL: hardTTL,
		now:     time.Now,
...
func (c *dnsCache) lookup(ctx context.Context, host string, lookup dnsLookupFunc) ([]string, error) {
	now := c.now()
	c.mu.Lock()
	entry, ok := c.entries[host]
	if ok && now.Before(entry.softExpiry) {
		ips := append([]string(nil), entry.ips...)
		c.mu.Unlock()
		return ips, nil
	}
	if ok && now.Before(entry.hardExpiry) {
		ips := append([]string(nil), entry.ips...)
		if !entry.refreshing {
			entry.refreshing = true
			c.entries[host] = entry
			go c.refresh(context.Background(), host, lookup)
		}
		c.mu.Unlock()
		return ips, nil
	}
	...
	return c.resolveAndStore(ctx, host, lookup)
}

func (c *dnsCache) refresh(ctx context.Context, host string, lookup dnsLookupFunc) {
	_, _ = c.resolveAndStore(ctx, host, lookup)
}

func (c *dnsCache) resolveAndStore(ctx context.Context, host string, lookup dnsLookupFunc) ([]string, error) {
	addrs, err := lookup(ctx, host)
	if err != nil {
		c.clearRefreshing(host)
		return nil, err
	}
	if len(addrs) == 0 {
		c.clearRefreshing(host)
	...
	ips := append([]string(nil), addrs...)
	now := c.now()

	c.mu.Lock()
	c.entries[host] = dnsCacheEntry{
		ips:        ips,
		softExpiry: now.Add(c.softTTL),
		hardExpiry: now.Add(c.hardTTL),
	}
	c.mu.Unlock()
	return ips, nil
}

func (c *dnsCache) clearRefreshing(host string) {
	c.mu.Lock()
...
func dialContextTCPNoDelay(ctx context.Context, s *stack.Stack, addr tcpip.FullAddress, network tcpip.NetworkProtocolNumber) (*gonet.TCPConn, error) {
	var wq waiter.Queue
...
func (tnet *Net) ConfigureCubic() {
	cc := tcpip.CongestionControlOption("cubic")
	tnet.stack.SetTransportProtocolOption(tcp.ProtocolNumber, &cc) //nolint:errcheck
	moderateBuf := tcpip.TCPModerateReceiveBufferOption(true)
	tnet.stack.SetTransportProtocolOption(tcp.ProtocolNumber, &moderateBuf) //nolint:errcheck
	minRTO := tcpip.TCPMinRTOOption(20 * time.Millisecond)
	tnet.stack.SetTransportProtocolOption(tcp.ProtocolNumber, &minRTO) //nolint:errcheck
	noDelay := tcpip.TCPDelayEnabled(false)
	tnet.stack.SetTransportProtocolOption(tcp.ProtocolNumber, &noDelay) //nolint:errcheck
}
